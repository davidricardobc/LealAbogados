...
import { type LaborChatResult } from "@/lib/legal/labor-chat";
      ...
      {isOpen ? (
        ...
        >
          ...
          <div className="border-t border-ink/10 bg-white px-4 py-3">
            {lastResponse ? (
              <div className="mb-3 flex flex-wrap gap-2 text-[0.68rem] font-semibold uppercase tracking-[0.08em] text-muted">
                <span className="border border-ink/10 px-2 py-2">{caseShortLabelByType[lastResponse.lead.caseType]}</span>
                <span className="border border-ink/10 px-2 py-2">{urgencyShortLabelByType[lastResponse.lead.urgency]}</span>
                <span className="border border-ink/10 px-2 py-2">{roleShortLabelByType[lastResponse.lead.role]}</span>
              </div>
            ) : null}
